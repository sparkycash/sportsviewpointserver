...
export async function filterNewLinksOld(links) {
  const pool = await connectToSupabase();
  const tableName = process.env.ARTICLES || "articles";

  `select * from ${tableName} where original_link =$links.`
...
export async function filterNewLinksNewwer(links, category) {
 // console.log(links)
  const pool = await connectToSupabase();
  const tableName = process.env.ARTICLES || "articles";

  // If nothing passed, return empty
  if (!Array.isArray(links) || links.length === 0) {
    return { success: true, data: [] };
  }
  ...
  try {
    for (const item of links) {
      ...
      const { rows } = await pool.query(query, [linkUrl, category]);

      // If no rows returned → new link
      if (rows.length === 0) {
        results.push(item);
      }
    }

    console.log(`✅ Found ${results.length} new link(s).`);
...
export async function filterNewLinks(links, category, selectedDate) {
  ...
  // ---- DATE FIX (no timezone shifting) ----
  // Force midnight LOCAL time to avoid UTC conversion
  const parsedDate = new Date(`${selectedDate} 00:00:00`);
  console.log("Parsed Date:", parsedDate);

  if (isNaN(parsedDate)) {
    return { success: false, data: [], message: "Invalid date format" };
  }
  ...
  try {
    ...
    const previousLinks = rows.map(r => JSON.parse(r.original_link));
      console.log(`Found ${previousLinks.length} existing articles for this date`);
  console.log("LINK COUNT",links.length)


   // ---- FILTER NEW LINKS ----
    const newLinks = links.filter(link => {
      return !previousLinks.some(prev => prev.link === link.link);
    });
